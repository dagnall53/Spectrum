...
# Import your LOCAL driver package
...
class EmcScanner(QtWidgets.QMainWindow):
    ...
    def __init__(self):
        ...
        # Sweep centers
        self.centers = np.arange(START_FREQ, STOP_FREQ, STEP_HZ, dtype=np.float64)
    ...
    def set_preset(self, mode):
        global START_FREQ, STOP_FREQ, STEP_HZ, SAMPLE_RATE, GAIN
    ...
    def start_continuous(self):
        ...
        self.freq_axis = np.full(NFFT, np.nan, dtype=np.float64)
        self.power_axis = np.full(NFFT, -200.0, dtype=np.float64)
    ...
    def sweep_loop(self):
        ...
        for i, fc in enumerate(self.centers):
            ...
            # ---- Progress logging with bandwidth ----
            bw = SAMPLE_RATE
            f_low = fc - bw/2
            f_high = fc + bw/2

            self.log(
                f"FAST sweep: {i+1}/{len(self.centers)}  "
                f"fc={fc/1e6:.6f} MHz  "
                f"band={f_low/1e6:.6f}–{f_high/1e6:.6f} MHz"
            )
            
            actual_fc = self.sdr.get_center_freq()
            if abs(actual_fc - fc) > 100:
                print("WARNING: tuner did NOT move! DLL mismatch.")

            n_samples = NFFT * 2
            samples = self.sdr.read_samples(n_samples)
            if np.allclose(samples, samples[0]):
                print("WARNING: samples are constant — SDR not streaming.")
            samples = samples.astype(np.float32) / 128.0
            spec = np.fft.fftshift(np.fft.fft(samples[:NFFT] * window))
            spec = spec / NFFT
            psd  = 20 * np.log10(np.abs(spec) + 1e-12)
            # correction to approximate to readings SDRSHARP gives
            psd = psd + 33.5
